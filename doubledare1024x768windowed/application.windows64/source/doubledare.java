...
public class doubledare extends PApplet {
...
public void checkTime() {
  ...
  if (tdisp.contains(":") && count == 1) {
    ...
    t1duration = (60 * PApplet.parseInt(m[0])) + PApplet.parseInt(m[1].substring(0,m[1].length()-1));
    if (!started) {
      lastTime = t1duration;
    }
  } else if (tdisp.matches("[0-9]+") && tdisp.length() > 0) {
    t1duration = PApplet.parseInt(tdisp);
    if (!started) {
      lastTime = t1duration;
    }
  }
...
public void draw() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "doubledare" };
    if (passedArgs != null) {
